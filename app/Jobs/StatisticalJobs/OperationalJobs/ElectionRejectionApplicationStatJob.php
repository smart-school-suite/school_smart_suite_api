...
class ElectionRejectionApplicationStatJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $applicationDetails = ElectionApplication::where("school_branch_id", $this->schoolBranchId)
         ...
         $totalRegectionCountKpi = $kpis->get("total_application_rejection_count");
        if($applicationDetails){
             $this->upsertElectionApplicationStat(
                $this->schoolBranchId,
                $totalRegectionCountKpi,
                $year
             );
        }
        else {
            Log::warning("StatType 'total_application_rejection_count' not found. Skipping Election Application Count.");
        }
