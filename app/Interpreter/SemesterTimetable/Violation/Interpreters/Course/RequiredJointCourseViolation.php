...
class RequiredJointCourseViolation implements ViolationInterpreter
{
    ...
    public function explain(array $blocker): string
    {
        ...
        return "Required Joint Course Violation: The fixed joint course session on {$entity['day']} at {$entity['start_time']} to {$entity['end_time']} for course {$course->course_title} with teacher {$teacher->name} in hall {$hall->name} conflicts with the requested session on {$conflict['day']} at {$conflict['start_time']} to {$conflict['end_time']} for course {$conflictingCourse->course_title}.";
    }
}
