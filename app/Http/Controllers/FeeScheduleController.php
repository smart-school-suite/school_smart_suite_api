...
class FeeScheduleController extends Controller
{
    ...
    {
        $currentSchool = $request->attributes->get('currentSchool');
        $feeSchedule = $this->feeScheduleService->getFeeScheduleStudentId($currentSchool, $studentId);
    ...
    {
        Log::info("This Code Ran");
        $currentSchool = $request->attributes->get('currentSchool');
        $authStudent = $this->resolveUser();
        $feeSchedule = $this->feeScheduleService->getStudentFeeSchedule($currentSchool, $authStudent);
        return ApiResponseService::success("Student Fee Schedule Fetched Successfully", $feeSchedule, null, 200);
    }
    ...
    public function getStudentFeeScheduleLevelId(Request $request, $levelId){
        ...
        $currentSchool = $request->attributes->get('currentSchool');
        $authStudent = $this->resolveUser();
        $feeSchedule = $this->feeScheduleService->getStudentFeeScheduleLevelId($currentSchool, $authStudent, $levelId);
        return ApiResponseService::success("Student Fee Schedule Fetched Successfully", $feeSchedule, null, 200);
    }
