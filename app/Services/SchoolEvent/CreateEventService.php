        ...
        try {
            return DB::transaction(function () use ($currentSchool, $authenticatedUser, $data, $tags, $recipients) {
                ...
                $startDate = Carbon::parse($data['start_date']);
                 ...
                 if ($status === 'active') {
                   UpdateSchoolEventStatusJob::dispatch($schoolEventId, $currentSchool->id)->delay($endDate->add('minute', 1));
                   ...
                   UpdateSchoolEventStatusJob::dispatch($schoolEventId, $currentSchool->id)->delay($endDate);
                   UpdateSchoolEventStatusJob::dispatch($schoolEventId, $currentSchool->id)->delay($startDate);
                   UpdateSchoolEventVisibilityStatusJob::dispatch($currentSchool->id, $schoolEventId)->delay($publishedAt);
                //    CreateAnnouncementReciepientJob::dispatch($currentSchool->id, $recipients, $schoolEventId)
                //      ->delay($publishedAt);
                // SendAdminScheduledAnnouncementNotiJob::dispatch($schoolEventId, $authenticatedUser, $currentSchool->id);
                //if($publishedAt->greaterThan(now()->addMinutes(10))){
                // SendAdminAnnouncementScheduleReminderNotiJob::dispatch($schoolEventId, $authenticatedUser, $currentSchool->id)
                // ->delay($publishedAt->copy()->subMinutes(5));
                }
                //UpdateAnnouncementStatusJob::dispatch($schoolEventId, $currentSchool->id)->delay($expiresAt);}
