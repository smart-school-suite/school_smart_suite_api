...
class ValidateOtpService
{
    ...
    public function verifyOtp($tokenHeader, $otp)
    {
        ...
        if (!$user) {
            $otpRecord->delete();
            throw new AppException(
                "Associated user (actorable_id: {$otpRecord->actorable_id}) not found for valid OTP record.",
                500,
                "User Data Error 🚨",
                "A system error occurred: your verification code was valid, but we couldn't find your account to log you in. Please contact support immediately.",
                null
            );
        }
        ...
        if (!$apiKey) {
            throw new AppException(
                "School Branch API Key missing for school ID '{$user->school_branch_id}' during teacher login.",
                500,
