    ...
    public function run(): void
    {
        ...
        if (($handle = fopen($filePath, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle, 1000, ',')) !== false) {
                ...
                if (count($data) >= 2) {
                    $countries[] = [
                        'id' => $uuid, // Assign id from 1st column
                        'country' => $data[1], // Assign name from 2nd column
                        'code' => $data[2], // Assign name from 2nd column
                        'currency' => $data[3], // Assign name from 2nd column
                        'officail_language' => $data[4], // Assign name from 2nd column
